...
bool Prepared_statement::execute(String *expanded_query, bool open_cursor)
{
  ...
  */
  ...
  for (; global_list; global_list = global_list->next_global)
  {
    ...
    // but the view's tables may be EryDB
    if (global_list->view)
    {
      ...
      for (; view_list; view_list = view_list->next_global)
      {
        if (!(view_list->table && view_list->table->s && view_list->table->s->db_plugin))
          continue;
        if (view_list->table && view_list->table->isEryDB())
        {
          bHasEryDB = true;
          break;
        }
  ...
  if ((bHasEryDB && thd->get_command() == COM_STMT_EXECUTE) || (thd->lex->sql_command == SQLCOM_CALL))
  {
